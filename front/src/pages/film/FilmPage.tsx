...
export function FilmPage() {
	...
	return (
		<div className="@container bg-accent text-accent-foreground p-10">
			<Card className="@container  bg-secondary border-accent  p-10 gap-10  ">
				<div className="grid grid-flow-row [700px]:grid-rows-3 gap-10">
					<div className="col-span-2 flex justify-center rounded-lg">
						{film?.preview_path && (
							<img
								src={film?.preview_path}
								alt={`${film?.film_title}`}
								className="object-cover m-5 rounded-lg"
							/>
						)}
					</div>
				...
				<div className="mt-5 ">
					<div className=" ">
						{film?.film_path && <VideoPlayer src={film.film_path} />}
					</div>
				</div>
			</Card>
